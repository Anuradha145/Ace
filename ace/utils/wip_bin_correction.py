...
def execute_wip_bin_correction():
	...
	with lock:
		...
		plan = get_wip_blank_bin_balances()
		item_details = get_item_details({row["item_code"] for row in plan})
		plan, skipped = filter_unsupported_items(plan, item_details)

		if not plan:
			frappe.logger().info("No WIP blank-bin balances found for correction.")
			return {"status": "no_rows", "skipped": skipped}

		stock_entry = create_wip_bin_correction_entry(plan, item_details, skipped)
